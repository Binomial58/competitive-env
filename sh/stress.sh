...
for ((i = 0; i < COUNT; i++)); do
    ...
    if "${GEN_CMD[@]}" "$seed" > "$IN_FILE"; then
        ...
        echo "$(print_status GEN-ERROR 11) seed=$seed: generator が異常終了しました。" >&2
        cleanup_tmp
        exit 1
    fi
    ...
    if [ "$brute_status" -ne 0 ]; then
        echo "$(print_status BRUTE-ERROR 11) seed=$seed: brute が異常終了しました(exit $brute_status)。" >&2
        if [ -s "$BRUTE_ERR" ]; then
            cat "$BRUTE_ERR" >&2
        fi
    ...
    if [ "$main_status" -ne 0 ]; then
        echo "$(print_status RE 11) seed=$seed (${elapsed} ms, exit $main_status)"
        if [ -s "$MAIN_ERR" ]; then
            cat "$MAIN_ERR" >&2
        fi
    ...
    if [ -n "$tl" ] && [ "$elapsed" -gt "$tl" ]; then
        echo "$(print_status TLE 11) seed=$seed (${elapsed} ms > ${tl} ms)"
    ...
    if outputs_match "$BRUTE_OUT" "$MAIN_OUT"; then
        echo "$(print_status OK 11) seed=$seed (${elapsed} ms)"
    else
        echo "$(print_status MISMATCH 11) seed=$seed (${elapsed} ms)"
